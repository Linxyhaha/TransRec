    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if whole_word_ids is not None:
                ...
                inputs_embeds = inputs_embeds + whole_word_embeds #直接吧input_id的embedding和word embedding相加

        B, L = inputs_embeds.size()[:-1] # (bs, number of tokens)
        ...
        hidden_states = self.dropout(inputs_embeds) # (bs, # tokens, 768)
...
class P5(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = encoder_outputs[0] #(bs, 75, 768)

        if labels is not None and decoder_input_ids is None and decoder_inputs_embeds is None: # 刚开始generate
            # get decoder inputs from shifting lm labels to the right
            decoder_input_ids = self._shift_right(labels) # move to the right, add a zero at the beginning
        ...
        sequence_output = decoder_outputs[0] #(bs, # target tokens, 768)
        ...
        lm_logits = self.lm_head(sequence_output) # output size is vocab_size, which is pre-defined in the tokenizer based on vocab file. here the version being used is the vocab size of 32k

        loss = None
        if labels is not None:
            if reduce_loss:
                loss_fct = CrossEntropyLoss(ignore_index=-100) # 对无效label进行mask，id是-100
            else:
