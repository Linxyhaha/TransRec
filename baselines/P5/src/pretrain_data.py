...
class P5_Amazon_Dataset(Dataset):
    def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='toys', rating_augment=False, sample_type='random'): 
        ...
        if self.args.n_fewshot>0:
            self.sequential_data = ReadLineFromFile(os.path.join('your/path/to/dataset/', split, 'few_shot', f'{args.n_fewshot}', f'sequential_{args.n_fewshot}shot.txt'))
        else:
            self.sequential_data = ReadLineFromFile(os.path.join('data', split, 'sequential_data.txt'))
        item_count = defaultdict(int)
        user_items = defaultdict() # user item list dict

        for line in self.sequential_data: # for each user
            user, items = line.strip().split(' ', 1)
            items = items.split(' ') # str [1,2,3,4,5]
            items = [int(item) for item in items] # int [1,2,3,4,5]
        ...
        for i, meta_item in enumerate(self.meta_data):
            self.meta_dict[meta_item['asin']] = i # meta item id mapping
            
        print('compute_datum_info')
        self.total_length = 0
        self.datum_info = []
        self.compute_datum_info()
        print("data total length: ", self.total_length)
    ...
    def __getitem__(self, idx):
        ...
        datum_info_idx = self.datum_info[idx] # (i + curr, key, i // self.sample_numbers[key][0])
        ...
        elif task_name == 'sequential': # 如果sample number大于一，说明一个样本会被训练sample number次
        ...
        input_ids = self.tokenizer.encode(
                source_text, padding=True, truncation=True, max_length=self.args.max_text_length) # len 42
        tokenized_text = self.tokenizer.tokenize(source_text) # len 41
        whole_word_ids = self.calculate_whole_word_ids(tokenized_text, input_ids) # len 42
        assert len(whole_word_ids) == len(input_ids)
        
        target_ids = self.tokenizer.encode(
                target_text, padding=True, truncation=True, max_length=self.args.gen_max_length) # len 33
...
def get_loader(args, task_list, sample_numbers, split='toys', mode='train', 
    ...
    if 't5' in args.backbone:
        tokenizer = P5Tokenizer.from_pretrained(
            args.backbone, 
            max_length=args.max_text_length, # 512
    ...
    print(f"Amazon dataset length: {len(dataset)}")
    if distributed:
