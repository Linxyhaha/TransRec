...
from data import load_train_dataloaders, load_eval_dataloaders, load_data, load_fewshot_data
...
def trainer_fewshot(
    ...
    for epoch in range(args.epochs):
        ...
        if args.evaluation_method == "sequential_item":
            candidates = remapped_warm_items
            candidate_trie = Trie(
                [
                    [0] + tokenizer.encode("{}".format("item_" + candidate))
                    for candidate in candidates
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(test_loader_ww):
                output_ids = batch[3].to(args.gpu)
            ...
            ncdg_test_5 = ncdg_test_5 / test_total
            ncdg_test_10 = ncdg_test_10 / test_total
            logger.log("**** warm user warm item")
            logger.log("test hit @ 1 is {}".format(recall_test_1))
            logger.log("test hit @ 5 is {}".format(recall_test_5))
            logger.log("test hit @ 10 is {}".format(recall_test_10))
            logger.log("test ncdg @ 5 is {}".format(ncdg_test_5))
        ...
        if args.evaluation_method == "sequential_item":
            candidates = remapped_cold_items
            candidate_trie = Trie(
                [
                    [0] + tokenizer.encode("{}".format("item_" + candidate))
                    for candidate in candidates
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(test_loader_wc):
                output_ids = batch[3].to(args.gpu)
            ...
            ncdg_test_5 = ncdg_test_5 / test_total
            ncdg_test_10 = ncdg_test_10 / test_total
            logger.log("**** warm user cold item")
            logger.log("test hit @ 1 is {}".format(recall_test_1))
            logger.log("test hit @ 5 is {}".format(recall_test_5))
            logger.log("test hit @ 10 is {}".format(recall_test_10))
            logger.log("test ncdg @ 5 is {}".format(ncdg_test_5))
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(test_loader_ca):
                output_ids = batch[3].to(args.gpu)
            ...
            ncdg_test_5 = ncdg_test_5 / test_total
            ncdg_test_10 = ncdg_test_10 / test_total
            logger.log("**** cold user all items")
            logger.log("test hit @ 1 is {}".format(recall_test_1))
            logger.log("test hit @ 5 is {}".format(recall_test_5))
            logger.log("test hit @ 10 is {}".format(recall_test_10))
            logger.log("test ncdg @ 5 is {}".format(ncdg_test_5))
...
def main_worker(local_rank, args, logger):
    ...
    if args.n_fewshot<0:
        ...
        (
            ...
            remapped_warm_items,
            remapped_cold_items,
        ) = load_fewshot_data(args, tokenizer)
    ...
    if args.n_fewshot>0:
        test_loader_ww = load_eval_dataloaders(
            args,
            tokenizer,
            args.evaluation_method,
            "test",
        ...
        test_loader_wc = load_eval_dataloaders(
            args,
            tokenizer,
            args.evaluation_method,
            "test",
        ...
        test_loader_ca = load_eval_dataloaders(
            args,
            tokenizer,
            args.evaluation_method,
            "test",
        ...
        print("***", len(test_loader_ww), len(test_loader_wc), len(test_loader_ca))
