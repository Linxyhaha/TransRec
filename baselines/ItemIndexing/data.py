...
def load_fewshot_data(args, tokenizer):
    # set_seed(args)
    if args.data_order == "remapped_sequential":
        if args.remapped_data_order == "original":
            data_dir = args.data_dir + args.task + f"/remapped_sequential_data_{args.n_fewshot}shot.txt"
            with open(data_dir, "r") as f:
                data = f.read()
            data = data.split("\n")[:-1]
        elif args.remapped_data_order == "short_to_long":
    ...
    # history for test
    warm_cold_info = np.load(args.data_dir + args.task + f"/warm_cold_info_{args.n_fewshot}.npy", allow_pickle=True).item()
    warm_item = list(warm_cold_info["warm_item"])

    warm_cold_info = np.load(args.data_dir + args.task + f"/warm_cold_info_{args.n_fewshot}.npy", allow_pickle=True).item()
    cold_item = list(warm_cold_info["cold_item"])

    test_seq_ww = []
    test_seq_wc = []
    test_seq_ca = []
    for one_user in tqdm(data): # warm user
        splittion_point = one_user.index(" ")
        user = one_user[:splittion_point]
        items = one_user[splittion_point + 1 :].split(" ")
        if args.item_representation != "None":
        ...
        users.append(user) # warm user 
        val_sequence.append(remapped_items[:-1])

        if items[-1] in warm_item:
            test_seq_ww.append(remapped_items)

        if items[-1] in cold_item:
            test_seq_wc.append(remapped_items)



    with open(args.data_dir+ args.task + f"/remapped_sequential_data_{args.n_fewshot}shot_cu.txt", "r") as f:
        data = f.read()
    data = data.split("\n")[:-1]

    for one_user in tqdm(data): # cold user
        splittion_point = one_user.index(" ")
        user = one_user[:splittion_point]
        items = one_user[splittion_point + 1 :].split(" ")
        if args.item_representation != "None":
        ...
        if items[-1] in warm_item:
            test_seq_ca.append(remapped_items)


    # if args.item == "all":
            
    # save all items with full data
    with open(data_dir, "r") as f:
        full_data = f.read()
    full_data = full_data.split("\n")[:-1]

    for one_user in tqdm(full_data):
        splittion_point = one_user.index(" ")
        user = one_user[:splittion_point]
        items = one_user[splittion_point + 1 :].split(" ")
        if args.item_representation != "None":
    ...
    warm_cold_info = np.load(args.data_dir + args.task + f"/warm_cold_info_{args.n_fewshot}.npy", allow_pickle=True).item()
    warm_item = list(warm_cold_info["warm_item"])
    remapped_items = [remap_fct(item) for item in warm_item]
    warm_items += items
    remapped_warm_items += remapped_items

    cold_items = []
    remapped_cold_items = []
    warm_cold_info = np.load(args.data_dir + args.task + f"/warm_cold_info_{args.n_fewshot}.npy", allow_pickle=True).item()
    cold_item = list(warm_cold_info["cold_item"])
    remapped_items = [remap_fct(item) for item in cold_item] 
    cold_items += items
    remapped_cold_items += remapped_items
    ...
    print(len(test_seq_ww), len(test_seq_wc), len(test_seq_ca))
    print(len(remapped_all_items), len(remapped_warm_items), len(remapped_cold_items))
    return (
        users,
        all_items,
        warm_items,
