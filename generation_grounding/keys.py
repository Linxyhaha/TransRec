...
from generation_grounding import FMIndex
...
def aggregate_evidence(ngrams_and_scores: List[Tuple[List[int], float]], unigram_scores: Optional[List[float]] = None,
                       ...
                       single_key_add_unigrams=False, unigrams_ignore_free_places=False, tokenizer=None, bias_all=0, bias_title=0, bias_id=0, bias_attribute=6) -> Tuple[List[int], List[float]]:
    ...
    for i in range(len(ngrams_and_scores)):
        ...
        sr = min(sr + bias_all, -1.0) 
        if ngram[0] == 47385: # attribute bias
            sr = min(sr+bias_attribute, -1.0)
        elif ngram[0] == 45056: # id bias
            sr = min(sr+bias_id, -1.0) 
        else: # title bias
            sr = min(sr+bias_title, -1.0)
        ...
        if count == 0:
            ...
            if ngram[0]==47385:
                snr = math.log((count + smoothing) / (100*(ntokens + smoothing)) )
    ...
    for doc, doc_info in first_stage.items(): # for each item
